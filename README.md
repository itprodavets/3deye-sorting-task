...
<!-- TESTS:START -->
> Auto-generated by `.github/workflows/benchmark.yml` on 2026-04-15 10:47 UTC.
...
>
> Each cell is the **min of 2 timed runs after 1 warmup** (warmup primes page
...
|------|----------|-------|-------:|--------:|------|------------|----------|--------|
| 10MB | stream | 388983 | 1 | 4 | 0.43 s | 23 MB/s | 85 MB | ok |
| 10MB | mmf | 388983 | 1 | 4 | 0.38 s | 27 MB/s | 71 MB | ok |
| 10MB | shard | 388983 | 1 | 4 | 0.44 s | 23 MB/s | 84 MB | ok |
| 50MB | stream | 1945702 | 1 | 4 | 1.66 s | 30 MB/s | 133 MB | ok |
| 50MB | mmf | 1945702 | 1 | 4 | 1.52 s | 33 MB/s | 207 MB | ok |
| 50MB | shard | 1945702 | 1 | 4 | 1.63 s | 31 MB/s | 131 MB | ok |
| 200MB | stream | 7782241 | 1 | 4 | 4.80 s | 42 MB/s | 422 MB | ok |
| 200MB | mmf | 7782241 | 1 | 4 | 5.08 s | 39 MB/s | 714 MB | ok |
| 200MB | shard | 7782241 | 1 | 4 | 4.68 s | 43 MB/s | 422 MB | ok |
| 1GB | stream | 39850773 | 1 | 4 | 24.79 s | 41 MB/s | 3.04 GB | ok |
| 1GB | mmf | 39850773 | 1 | 4 | 28.36 s | 36 MB/s | 3.41 GB | ok |
| 1GB | shard | 39850773 | 1 | 4 | 24.87 s | 41 MB/s | 3.05 GB | ok |
<!-- BENCH:END -->
...
2. **Final merge** (binary → text) wraps each chunk in a **`BinaryRawChunkReader`** that exposes text as a raw UTF-8 byte slice (`RawLineEntry.TextUtf8`) into a reusable buffer. `BinaryReader.ReadString()` is skipped entirely — eliminating ~4.5 billion per-record string allocations at 100 GB scale. The order is UTF-8 lexicographic byte order, equivalent to Unicode code-point order — **not** `StringComparison.Ordinal` (which is UTF-16 code-unit order and disagrees with UTF-8 on the supplementary plane). `LineEntry.CompareTo` takes an ASCII fast path (`Ascii.IsValid` + `string.CompareOrdinal`, both SIMD-vectorized) — safe because UTF-16 code units equal UTF-8 bytes 1:1 for `U+0000..U+007F`, so `CompareOrdinal` yields the same order as UTF-8 byte comparison — and falls back to `Rune.EnumerateRunes()` only when either side contains non-ASCII content. Either branch agrees with UTF-8 byte order, so every strategy sorts byte-for-byte identically.
...
- **Stream Phase 1** (`LineEntry`): first 8 raw UTF-8 bytes packed as a big-endian `ulong` — same sort key as `EntryIndex` and `RawLineEntry`, so all three strategies agree byte-for-byte. Resolves most comparisons with a single integer compare; slow path gates on `Ascii.IsValid` and uses `string.CompareOrdinal` for ASCII text (SIMD, byte-identical to UTF-8 order when every char is `≤ U+007F`), falling back to `Rune.EnumerateRunes()` only when either side contains non-ASCII — preserving Unicode code-point order without per-char allocations.
